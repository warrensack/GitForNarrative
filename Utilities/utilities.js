 ...
 * 
 * Sample usage for node.js (where present directory contains the Utilities subdirectory):
 *    const makeUtilities = require('./Utilities/utilities.js');
 *    const utils = makeUtilities();
 * 
 * utils will then be an object containing all of the utility methods
 * that can be used, for example, like this: 
 *    utils.keys({1: 2, 3: 4});
 *    which returns ["1","3"]
 * 
 */
...
function makeUtilities() {
  ...
  function copyIt(e) {
    ...
    else { ppToConsole("copyIt: Unknown element: " + e); }
  }
  ...
  //
  // ppToBrowser(string): print to current browser window
  //
  function ppToBrowser(str) {
    let formatted = pp(str);
  ...
  //
  // ppToConsole(string): pretty print to current browser console
  //
  ...
  function ppObject(obj, indent) {
    ...
    for (let i = 0; i < ks.length; i++) {
      str = indent + "\"" + ks[i] + "\": ";
      if (i < (ks.length - 1)) { separator = ","; }
      else { separator = ""; }
      if ((typeOf(obj[ks[i]]) !== 'object')
        && (typeOf(obj[ks[i]]) !== 'array')) {
  ...
  function renameVars(x, id) {
    if ((isEmpty(x)) || (x === '') || (x === null)
      || (isAnonymousVariable(x))) {
      return (x);
    }
    if (typeOf(x) === 'object') {
      for (key in x) {
  ...
  function isClauseTypeWithArgsList(obj, clauseType) {
    ...
    let ks = utils.keys(obj);
    let vs = utils.values(obj);
    if ((ks.length === 1)
      && (ks[0] === clauseType)
      && (utils.typeOf(vs[0]) === 'array')) {
      return (true);
    }
    return (false);
  }
  ...
  ////
  //// Syntax for lists in JSON
  ////
  //// {"list": ["1","2"]} ==> {"cons": {"first": "1", "rest": {"cons": {"first": "2", "rest": "nil"}}}}
  ////
  ...
  function listToAssertion(a) {
    ...
    else { return ({ "cons": { "first": a.shift(), "rest": listToAssertion(a) } }); }
  }
  ...
  function occursIn(variable, pattern) {
    ...
    if (typeOf(pattern) === 'object') {
      if (pattern[variable] !== undefined) { return (true); }
      for (let key in pattern) {
        if (occursIn(variable, pattern[key])) { return (true); }
      }
    }
    if (typeOf(pattern) === 'array') {
      for (let i = 0; i < pattern.length; i++) {
        if (occursIn(variable, pattern[i])) { return (true); }
      }
    }
    return (false);
  }
  ...
  function unifyPairs(obj1, obj2, bindings) {
    ...
    for (key in obj1) {
      if (obj2[key] !== undefined) {
        if (!(unifyPatterns(obj1[key], obj2[key], bindings))) { return (false); }
    ...
    for (let i = 0; i < sharedKeys.length; i++) {
      ...
      if (bindings[key] !== undefined) {
        ...
        delete bindings[key];
        if (!(unifyPairs(dpair, bpair, bindings))) { return (false); }
        else { bindings[key] = bpair[key]; }
      }
    }
    for (key in obj2) {
  ...
  function unifyPatterns(pattern1, pattern2, bindings) {
    ...
    if ((typeOf(pattern1) === 'array')
      && (typeOf(pattern2) === 'array')) {
      if (pattern1.length === pattern2.length) {
        let result = true;
        for (let i = 0; i < pattern1.length; i++) {
          if (result) { result = unifyPatterns(pattern1[i], pattern2[i], bindings); }
        }
        return (result);
      }
      else { return (false); }
  ...
  function containsVariable(pattern) {
    ...
    if (typeOf(pattern) === 'object') {
      for (let key in pattern) {
        if (containsVariable(pattern[key])) { return (true); }
      }
    }
    if (typeOf(pattern) === 'array') {
      for (let i = 0; i < pattern.length; i++) {
        if (containsVariable(pattern[i])) { return (true); }
      }
    }
    return (false);
  }
  ...
  function equal(thing1, thing2) {
    if (containsVariable(thing1) || containsVariable(thing2)) { return (false); }
    if (unifyPatterns(thing1, thing2, {})) { return (true); }
    else { return (false); }
  }
  ...
  // positions (i.e., if they can be unified without creating any additions
  // to the bindings).
  //
  function necessarilyUnify(pat1, pat2, bindings) {
    let freshBindings = copyIt(bindings);
    let i1 = instantiate(copyIt(pat1), freshBindings);
    let i2 = instantiate(copyIt(pat2), freshBindings);
    let uBindings = {};
    let result = unifyPatterns(i1, i2, uBindings);
    if (result && isEmpty(uBindings)) { return (true); }
    else { return (false); }
  }
  ...
  function setIntersection(s1, s2) {
    let intersection = [];
    for (let key in s1) {
      if (s2[key] !== undefined) { intersection.push(key); }
    }
  ...
  function setDifference(s1, s2) {
    let difference = [];
    for (let key in s1) {
      if (s2[key] === undefined) { difference.push(key); }
    }
  ...
  function makeSearch() {
    ...
    function expandSearchState(sstate, expander) {
      let newSStates, successors;
      if (sstate["depth"] == undefined) { sstate["depth"] = 0; }
      if (sstate["successors"] == undefined) {
        sstate["successors"] = [];
        successors = [];
        newSStates = expander(sstate);
        for (let i = 0; i < newSStates.length; i++) {
    ...
    function searchSuccess(sstate, isSuccess) {
      if (isSuccess(sstate)) { sstate["success"] = true; }
    ...
    function allUnexpandedStates() {
      let unexpanded = [];
      for (let id in searchStates) {
        if ((searchStates[id]["successors"] == undefined)
          && !searchStates[id]["success"]) {
    ...
    function search(scorer, expander, isSuccess) {
      let queue = allUnexpandedStates();
      if (isEmpty(queue)) { return ({}); }
      else {
        for (let i = 0; i < queue.length; i++) { scoreSearchState(queue[i], scorer); }
        ...
        */
        if (searchSuccess(queue[0], isSuccess)) { return (queue[0]); }
        else {
